...
void flist_unlock(void)
{
	if (pthread_rwlock_unlock(&files_lock) != 0) {
		multi_debug(MULTI_DEBUG, "E error unlock flist\n");
	}
	multi_debug(MULTI_DEBUG, "< unlock flist\n");
}
...
struct flist ** flist_items_by_eq_name(struct flist * info)
{
	...
	multi_debug(MULTI_INFO, "flist_items_by_eq_name: %s\n", info->name);
...
static void delete_item(struct flist *item, int locked)
{
	...
	flist_foreach(next) {
		if (next == item) {
			...
			multi_debug(MULTI_DEBUG, "delete_item: %s (%s)\n",
