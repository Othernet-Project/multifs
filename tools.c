...
int get_free_dir(void)
{
	...
	for (max = i = 0; i < multi.cdirs; i++) {

		if (statvfs(multi.dirs[i], &stf) != 0)
			continue;
		...
		if (multi.move_limit <= 100) {
			...
			if (multi.move_limit != 100) {
				...
				if (multi.move_limit != 99) {
					perclimit *= multi.move_limit + 1;
					perclimit /= 100;
				}
...
static int find_free_space(off_t size)
{
	...
	for (max=-1,i=0; i<multi.cdirs; i++)
	{
		if (statvfs(multi.dirs[i], &stf)!=0) continue;
		fsblkcnt_t space  = stf.f_bsize;
		space *= stf.f_bavail;

		if (space>size+multi.move_limit) return i;
...
static int reopen_files(struct flist * file, const char *new_name)
{
	...
	multi_debug(MULTI_INFO, "reopen_files: %s -> %s\n",
...
int move_file(struct flist * file, off_t wsize)
{
	...
	multi_debug(MULTI_MSG, "move_file: %s\n", file->real_name);
	...
	if (fstat(file->fh, &st) != 0) {
		multi_debug(MULTI_MSG, "move_file: error stat %s: %s\n",
			from, strerror(errno));
		return -errno;
	}
	...
	if (st.st_nlink > 1) {
		multi_debug(MULTI_MSG, "move_file: cannot move "
	...
	if (space > size) {
		multi_debug(MULTI_MSG, "move_file: we have enough space\n");
		return 0;
	}

	if ((dir_id=find_free_space(size)) == -1) {
		multi_debug(MULTI_MSG, "move_file: can not find space\n");
		return -1;
	}
	...
	to = create_path(multi.dirs[dir_id], file->name);
	if (!(output = fopen(to, "w+"))) {
		ret = -errno;
		multi_debug(MULTI_MSG, "move_file: error create %s: %s\n",
				to, strerror(errno));
		free(to);
		fclose(input);
		return(ret);
	...
	multi_debug(MULTI_MSG, "move_file: move %s to %s\n", from, to);
	...
	multi_debug(MULTI_MSG, "move_file: done move data\n");
	fclose(input);
	...
	multi_debug(MULTI_MSG, "move_file: %s -> %s: done, code=%d\n",
		from, to, ret);
	free(to);
	free(from);
	return ret;
...
char * create_path(const char *dir, const char * file)
{
	...
	/*     multi_debug(MULTI_DEBUG, "create_path: %s\n", path); */
	return(path);
}
...
char * find_path(const char *file)
{
	...
	{
		char *path=create_path(multi.dirs[i], file);
...
int create_parent_dirs(int dir_id, const char *path)
{
	...
	char *path_parent=create_path(multi.dirs[dir_id], parent);
	struct stat st;
